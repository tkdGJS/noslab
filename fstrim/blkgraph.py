...
#    plt.rc('figure', titlesize=1000) # figure title 폰트 크기
    ...
    for i in range(len(fio_off)):
        plt.plot([fio_time[i], fio_time[i]], [fio_off[i], fio_ln[i]], color='red',linewidth=8)



    plt.plot(fio_time,fio_off,'o',markersize=8,color='red')

    plt.plot(fio_time,fio_ln,'o',markersize=8,color='red')


    for i in range(len(fstrim_off)):
        plt.plot([fstrim_time[i], fstrim_time[i]], [fstrim_off[i], fstrim_ln[i]], color='blue',linewidth=8)



    plt.plot(fstrim_time,fstrim_off,'o',markersize=8,color='blue')

    plt.plot(fstrim_time,fstrim_ln,'o',markersize=8,color='blue')

    for i in range(len(meta_off)):
        plt.plot([meta_time[i], meta_time[i]],[meta_off[i], meta_ln[i]], color='green',linewidth=8)

    plt.plot(meta_time,meta_off,'x',markersize=8,color='green')

    plt.plot(meta_time,meta_ln,'x',markersize=8,color='green')
...
#    plt.rc('figure', titlesize=1000) # figure title 폰트 크기
    ...
    for i in range(len(offset)):
        plt.plot([time[i], time[i]],[offset[i], length[i]], color='red',linewidth=8)



    plt.plot(time,offset,'o',markersize=8,color='red')

    plt.plot(time,length,'o',markersize=8,color='red')
...
#    plt.rc('figure', titlesize=1000) # figure title 폰트 크기
    ...
    for i in range(len(offset)):
        plt.plot([time[i], time[i]],[offset[i], length[i]], color='blue',linewidth=8)



    plt.plot(time,offset,'o',markersize=8,color='blue')

    plt.plot(time,length,'o',markersize=8,color='blue')
...
if fs=='ext4':
    ...
    trace(fs='f2fs', proc='0', event='C', rwbs='W', m_event='D', m_rwbs='WSM', f_proc='fstrim', f_event='I', f_rwbs='DS')
