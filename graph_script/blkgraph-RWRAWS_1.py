...
ymin, ymax = 0, 67108864/32
y_padding = 0.1 * (ymax - ymin)



def fio_graph(offset, length, time, offset2, length2, time2, offset3, length3, time3, offset4, length4, time4, offset5, length5, time5, fs):

    name = str(fs)+'RWRAWS_1'
...
#    plt.rc('figure', titlesize=1000) # figure title 폰트 크기
    ...
    plt.xlabel('Time (sec)')
    plt.ylabel('Logical Block Address (GiB)')
    plt.ylim(ymin * 512 / (1024 * 1024 * 1024), ymax * 512 / (1024 * 1024 * 1024))
    
    offset = [o * 512 / (1024 * 1024 * 1024) for o in offset]
    offset2 = [o * 512 / (1024 * 1024 * 1024) for o in offset2]
    offset3 = [o * 512 / (1024 * 1024 * 1024) for o in offset3]
    offset4 = [o * 512 / (1024 * 1024 * 1024) for o in offset4]
    offset5 = [o * 512 / (1024 * 1024 * 1024) for o in offset5]

    plt.plot(time, offset, 'x', markersize=4, label=f"RA", color='blue')
    plt.plot(time2, offset2, 'x', markersize=4, label=f"WS", color='red')
    plt.plot(time5, offset5, 'x', markersize=4, label=f"WSM", color='green')
    plt.plot(time3, offset3, 'o', markersize=4, label=f"R", color='blue')
    plt.plot(time4, offset4, 'o', markersize=4, label=f"W", color='red')    
...
#====================
    ...
    fio_graph(fio_off,fio_ln,fio_time,fio_off2,fio_ln2,fio_time2,fio_off3,fio_ln3,fio_time3,fio_off4,fio_ln4,fio_time4,fio_off5,fio_ln5,fio_time5,fs)
